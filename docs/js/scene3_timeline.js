...
/* =============================================
   PHOTO GENERATOR
   (Deterministic procedural city skyline)
   ============================================= */
...
/* Japan-specific curated taglines */
...
/* Japan-specific curated detail narratives */
const JAPAN_CONTEXT = {
  1995: {
...
function curatedContext(year, yd) {
  ...
  const deathCtx = yd.deaths > 5000
    ? 'A catastrophic toll — concentrated in one or a few high-impact events.'
    : yd.deaths > 500 ? 'Concentrated in a small number of high-impact events.'
    : yd.deaths > 0   ? 'Distributed across multiple smaller-scale incidents.'
    : 'No fatalities were recorded in official reports for this year.';

  const affectedCtx = yd.affected > 500000
    ? 'Hundreds of thousands required assistance, shelter, or evacuation.'
    : yd.affected > 0 ? 'Includes displaced, injured, and otherwise impacted persons.'
    : 'Affected figures were not reported or fell below the registration threshold.';

  const damageCtx = yd.damage > 5
    ? 'Significant economic losses triggered reconstruction programmes.'
    : yd.damage > 0 ? 'Estimated direct economic losses, adjusted to USD.'
    : 'Economic damage figures were not available for this period.';
  ...
  return {
    deaths:    deathCtx,
    affected:  affectedCtx,
    damage:    damageCtx,
    narrative: `${year} recorded ${yd.total} catalogued event${yd.total !== 1 ? 's' : ''} in ${COUNTRY_NAME}, dominated by ${typeName} incidents.${deathStr}${damageStr} The year sits within the broader pattern of ${era}.`
  };
}
...
let stageCards = []; // Will now hold Three.js objects
let totalDepth = 0;
const SPACING = 5; // Scaled down for 3D space

// UI Elements (Keep these from your old code)
...
function initThreeJS() {
  ...
  container.innerHTML = ''; // Clear out any old HTML

  scene = new THREE.Scene();
  // Add dark, moody museum fog
...
/* =============================================
   CARD INTERFACE (Glassmorphism UI)
   ============================================= */
async function createCardTexture(yd) {
  try {
    // 1. Create a high-res offscreen canvas (4:5 ratio)
    const canvas = document.createElement('canvas');
    canvas.width = 960;
    canvas.height = 1200; 
    const ctx = canvas.getContext('2d');

    // 2. Draw Glassy Background & Rounded Corners (100% Browser Safe)
    ...
    // Save state and clip to keep SVG inside rounded corners
    ctx.save();
    ctx.clip(); 

    // 3. Load and Draw the SVG Cityscape Safely
    const svgURI = makePhoto(yd.year);
    const img = new Image();
    img.src = svgURI;

    // Await the image load, but FORCE resolve even if it fails
    await new Promise((resolve) => {
      img.onload = () => resolve();
      img.onerror = () => {
        console.warn(`SVG failed to load for ${yd.year}, drawing blank.`);
        resolve(); 
      };
    });
   
    ctx.drawImage(img, 0, 0, 960, 864); 

    // 4. Draw Gradient Fade (Blends the SVG into the card bottom)
    ...
    // Restore state so border doesn't get clipped
    ctx.restore();

    // 7. Draw the Glowing Border (Safe Way)
    ...
    console.error("FATAL ERROR building card for year", yd.year, error);
    // If it completely crashes, return a blank canvas so the script doesn't die!
...
/* =============================================
   CARD BUILDER (THREE.JS MESHES)
   ============================================= */
async function buildCards(yearDataArr) {
  initThreeJS();
  
  // Generate all textures safely
  ...
  yearDataArr.forEach((yd, i) => {
    ...
    scene.add(mesh);
    stageCards.push({ mesh: mesh, zPos: zPos, year: yd.year, data: yd });
  });
  ...
  scrollSpacer.style.height = `${maxScrollPixels + window.innerHeight + scrollBuffer}px`;
  
  setupRaycaster();
}

/* =============================================
   SCROLL / CAMERA RENDER LOOP
   ============================================= */
...
function tick() {
  ...
  // 1. Define how far back the camera should start (positive numbers move it back)
  const START_OFFSET = 20; 
  // 2. Subtract the scroll distance from that starting point
  const cameraZ = START_OFFSET - (sc * 0.02);

  camera.position.z = cameraZ;

  // Find the closest card for HUD updates
  let closest = stageCards[0];
  let bestDist = Infinity;

  for (const c of stageCards) {
    // Distance from camera (Negative means it's IN FRONT of the camera)
    const relZ = c.zPos - cameraZ; 
    
    // --- NEW: FADE OUT LOGIC ---
    // Change these numbers to adjust exactly when the card disappears
    const fadeStart = -12; // Start fading when 12 units away from camera
    const fadeEnd = -6;    // Completely invisible when 6 units away

    if (relZ > fadeEnd) {
      // It has passed the threshold (or is behind the camera)
      c.mesh.material.opacity = 0;
      c.mesh.visible = false; // Turn off rendering to save GPU
    } else {
      c.mesh.visible = true;
      if (relZ > fadeStart) {
        // It is in the "fade zone" - calculate smooth opacity
        let fadeAmount = (relZ - fadeStart) / (fadeEnd - fadeStart);
        c.mesh.material.opacity = 1 - fadeAmount;
      } else {
        // It is safely far away
        c.mesh.material.opacity = 1;
      }
    }
    // ---------------------------

    // Existing logic to find the closest card for your HTML HUD
  ...
  if (hudYear.textContent !== String(closest.year)) {
    ...
    // You can update other floating HTML HUD elements here based on `closest.data`
...
/* =============================================
   RAYCASTER (HANDLING CLICKS IN 3D)
   ============================================= */
...
function setupRaycaster() {
  ...
  window.addEventListener('click', (event) => {
    // Only check clicks if modal is NOT open
    ...
    for (let i = 0; i < intersects.length; i++) {
      if (intersects[i].object.userData.isCard) {
        openDetail(intersects[i].object.userData.year);
        break; // Stop after finding the first clicked card
      }
    }
  });
  
  // Optional: Change cursor to pointer when hovering over cards
  window.addEventListener('mousemove', (event) => {
    ...
    raycaster.setFromCamera(mouse, camera);
    const intersects = raycaster.intersectObjects(scene.children);
    document.body.style.cursor = (intersects.length > 0 && intersects[0].object.userData.isCard) ? 'pointer' : 'default';
  });
}
...
document.getElementById('closeDetail').addEventListener('click', (e) => {
  closeDetail();
  e.stopPropagation(); // Stops the click from reaching the Raycaster
});

// 2. Close when clicking the dark background (outside the card)
...
function closeDetail() {
  ...
  // EXTRA SAFETY: Turn clicks back on for the 3D world
...
function openDetail(year) {
  ...
  // Real photo logic with fallback
  const img = new Image();
  ...
  /* ==========================================
     D3.JS DONUT CHART
     ========================================== */
  const data = Object.entries(yd.breakdown)
    .filter(([, n]) => n > 0)
    .map(([k, n]) => ({ name: k, value: n }));
  ...
  svg.append("g")
    ...
    .join("path")
      ...
      .attr("d", arc)
    .append("title")
      .text(d => `${typeLabel(d.data.name)}: ${d.data.value.toLocaleString()}`);
  ...
  document.getElementById('pieLegend').innerHTML = data
    .map(d => `
      <div class="item">
...
async function init() {
  /* Try loading the CSV from multiple relative paths */
  ...
  /* Get country name and region from CSV */
  ...
  /* Build global lookup for detail modal */
  ...
  document.getElementById('introTitle').innerHTML =
    `${COUNTRY_NAME},<br><em>over fifty years.</em>`;
  document.getElementById('introLede').textContent =
    `A walk down the museum hallway — ${yearDataArr.length} years of recorded disasters in ${COUNTRY_NAME}. Each exhibit drifts toward you, presents itself, and steps aside.`;
  ...
  document.getElementById('endCardText').textContent =
    `You've walked through the full record for ${COUNTRY_NAME}. Step into the country overview to see it all at once, or walk the corridor again.`;
